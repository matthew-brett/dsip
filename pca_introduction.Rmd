...
We are going to use the Premier League table data for this example:
...
While we're at it, let's convert the wages to units of 10 million GBPs, to make them a bit easier to read:
...
We will call the rows *samples* and the columns *features*.


To make things simpler, we will subtract the mean across samples from each
feature.  As each feature is one column, I need to subtract the mean of each column,
...
plt.scatter(X[:, 0], X[:, 1])
plt.axis('equal')
plt.xlabel('Defense (feature 1) (standard units)')
plt.ylabel('Forward (feature 2) (standard units)');
```
...
the 50 2D vectors in the *rows* of $\mathbf{X}$.
...
projected = c_values.dot(u_col.T)
projected
```
...
distances = np.sqrt(np.sum(remaining ** 2, axis=1))
distances
```
...
def line_projection(u, X):
    ...
    """
    u = u.reshape(2, 1)  # A column vector
...
plt.arrow(0, 0, u_best[0], u_best[1], width=0.01, color='r')
plt.annotate('$\hat{u_{best}}$', u_best, xytext=(20, 20),
