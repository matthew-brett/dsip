...
- we want likelihood to be high; we want to *maximise* the likelihood function (multiplying together the "matching" probabilities)
- we want log loss to be low; we want to *minimize* the loss function (aka the negative log likelihood/cost function/loss function)t{died})_i$mple.
\]
...
```{python}
# set the passenger's `survived` score
...
There is a bit of choice involved here as to which metric to favour, see this stats.stackexchange discussion for [more](https://stats.stackexchange.com/questions/578982/model-selection-aic-bic-and-cross-validation-gives-different-conclusion)
