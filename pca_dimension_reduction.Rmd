...
We can use these four columns instead of the orignal four columns as regressors, and this gives exactly the same model - in the sense that we get the same fitted values when we fit the model:
...
But now we've seen the scree-plot, we might conclude that only the first component is important, and we can discard the others.  We then just include the first component values a new model.
...
We still have a decent model fit with just the first component:
...
We could improve it a little by adding the second component:
...
Until we get the full R2 with all four components (which gives the same underling model as using the original four regressors):
