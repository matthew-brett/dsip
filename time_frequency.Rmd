...
The Fourier tranforms is therefore using all these basis functions - each basis function is a sinusoid.  The sinusoids become faster and faster in frequency as we go from first to last:
...
```{python}
ft_reconstructed = ft_multiplied.sum(axis=0)
plt.plot(ft_reconstructed)
```

As you can see, we still need to do some scaling to reconstruct the exact values of the signal, but we have the original signal shape:
