...
First, let's improt some libraries/functions to set up the page:
...
The dataset we will use for this page is a historical social science dataset about the Titanic disaster. Information about the dataset can be found [here](https://matthew-brett.github.io/cfd2020/data/titanic.html). A desciption of the variables in the dataset is below:
...
def plot_scatter(log_odds=False):
    ...
    if log_odds==True:
        ...
        pd.DataFrame({'fare':fare,
                     'log_odds_of_survival': log_odds_predictions_single_predictor}).plot.scatter('fare',
                                                                                                 'log_odds_of_survival',
                                                                                                  c=colors)
...
After fitting the model, the parameters come to us on the log odds scal. The mathematical notation for the log odds predictions is:

$ \text{log odds}(y_i == 1) = b_0 + b_1 * x_1$

Which in pythonic terms if:
...
These two cruves provide a graphical explanation of how the logistic regression cost function works.
...
In practice, to make the computations easier for a computer, we `minimize` the negative log-likehood. For each
...
We then add these log-likehoods together, and take the negative of the result (e.g. we just add a minus sign/multiply by minus 1).
...
We can modifiy our cost function to accept two predictors. To do this, we just
...
- we can inclue categorical predictors in the same way as for linear regression
...
### Accurary (for models with a categorical outcome variable)
...
```{python}
# calcuate the accuracy from the single predictor model
...
We then aim, through model comparison, to find the model that has the best "sweet spot" betwee complexity, and performance when predicting the unseen values in the test set:
...
# evauate the perform against the test data
...
survived_fare_age_cross_val = cross_val_score(LogisticRegression(),
                                        ...
                                        cv=5,
                                      scoring='accuracy')
# show the cross validaton result
