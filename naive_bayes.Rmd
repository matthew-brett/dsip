...
situations: the penguin is a Chinstrap, or the penguin is a Gentoo.  These are
mutually exclusive, so we add the probabilities to give the probability of getting a particular mass value $m$:
...
The probabilities for the two categories do not sum to 1, because the
probabilities $P(m | C)$, $P(m | G)$ do not sum to 1.  This in turn is because
we are taking discrete samples from the continuous Gaussian, so the sum of the
probabilities will depend on the samples.   We allow for this below by dividing
the two probabilities by their (< 0) sum, to normalize them to sum to 1.

```{python}
raw_posteriors = df[['p_c_given_m', 'p_g_given_m']]
posteriors = raw_posteriors.divide(raw_posteriors.sum(axis=1), axis=0)
posteriors
```
...
assert np.allclose(skl_posteriors, posteriors)
# Show scikit-learn's posterior probabilities.
pd.DataFrame(skl_posteriors, columns=list(posteriors))
```
