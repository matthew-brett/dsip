...
Compile Sklearn parameters, and use these to calculate fitted values with our function.  Compare, to show they are (near as dammit) similar.
...
Write the mean of the observed values $y$ as $\bar{y}$.   Then the total sum of squares is:
...
We can calculate by hand to show this gives the same answer.

```{python}
ss_total = ((quality - quality.mean()) ** 2).sum()
ss_total
```
...
```{python}
alpha = 1.0
# Set solver method and tolerance to get very close to sklearn value.
...
```{python}
def drop_and_predict(df, x_cols, y_col, to_drop_label):
    dropped_df = df.loc[to_drop_label:to_drop_label]  # Get data frame with single row.
    rest_df = df.drop(index=to_drop_label)  # Dataframe without dropped row.
    # Fit on everything but the dropped row.
    fit = sklm.LinearRegression().fit(rest_df[x_cols], rest_df[y_col])
    # Use fit to predict the dropped row.
    fitted = fit.predict(dropped_df[x_cols])
    return fitted[0]
```
...
```{python}
out_quality = quality.copy().rename('large_prediction')
for label in df.index:
    out_quality.loc[label] = drop_and_predict(df, ['Clarity', 'Easiness'], 'Quality', label)
out_quality
```
...
```{python}
out_quality_reduced = quality.copy().rename('reduced_prediction')
for label in df.index:
    out_quality_reduced.loc[label] = drop_and_predict(df, ['Clarity'], 'Quality', label)
out_quality_reduced
```
