...
$i$ is $y_i - \hat{y}_i$, and $SS_{\text{res}}$ is the sum of
...
The demominator for the standard $R^2$ statistic is the "total sum of squares" $SS_\text{tot}$, written as:
...
multiplication), we can also do the multiplication by $\frac{1}{n}$
...
We can calculate weights to weight each "Helpfulness" value by the number of professors.  Maybe we would do this on the basis that larger number of professors may give more reliable values.
...
Weighted regression differs from standard regression, only in weighting each *residual* $e_i = y - \hat{y}$ by some weight for that observation — call this $w_i$.
