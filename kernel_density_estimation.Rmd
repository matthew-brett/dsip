...
Notice this histogram is rather blocky. We could try increasing the number of bins, but we've only got 119 rows to play with:
...
# Black marks at the actual values.
plt.plot(bill_depth, tick_y, '|k', markeredgewidth=1);
nd_x = np.linspace(19, 22, 1000)
plt.plot(nd_x, max_nd.pdf(nd_x), 'r:');
```

Here we've replace the single point (the maximum) with a spread-out version of that point.
...
nd_x = np.linspace(
    values.min() - 4 * dist_sd,
    values.max() + 4 * dist_sd,
    1000
)
for point in values:
    nd = sps.norm(point, dist_sd)
    plt.plot(nd_x, nd.pdf(nd_x), 'r:')
```
...
This is the (shape of) our kernel density estimate.
...
# Divide by sum and  gap
kde = y_values / (np.sum(y_values) * delta_x)
```
